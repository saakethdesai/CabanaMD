...
void Mode::calculateSymmetryFunctionGroups(System *s, AoSoA_NNP nnp_data, t_neighbor neigh_list)
{
    ...
    auto calc_symm_op = KOKKOS_LAMBDA ( const int i, const int j, const int k)
    {
      int attype = type(i);
      ...
      for (int groupIndex = 0; groupIndex < countergtotal[attype]; ++groupIndex)
        {
          int memberindex0 = d_SFGmemberlist(attype,groupIndex,0);
          ...
          if (d_SF(attype,memberindex0,1) == 2)
          ...
          else if (d_SF(attype,memberindex0,1) == 3)
            {
              ...
              T_F_FLOAT dxik, dyik, dzik, dxjk, dyjk, dzjk;
              double lambda, zeta;
    ...
    Cabana::angular_neighbor_parallel_for( policy, calc_symm_op, neigh_list,
                                           Cabana::SerialNeighborOpTag(), "Mode:calculateSymmetryFunctionGroups" );
    Kokkos::fence();


    Kokkos::parallel_for ("Mode::scaleSymmetryFunctionGroups", s->N_local, KOKKOS_LAMBDA (const int i)
    {
      int attype = type(i);
...
void Mode::calculateForces(System *s, AoSoA_NNP nnp_data, t_neighbor neigh_list)
{
    ...
    Kokkos::RangePolicy<ExecutionSpace> policy( 0, s->N_local );
    auto calc_force_op = KOKKOS_LAMBDA (const int i, const int j, const int k)
    {
      int attype = type(i);
    ...
    Cabana::angular_neighbor_parallel_for( policy, calc_force_op, neigh_list,
                                           Cabana::SerialNeighborOpTag(), "Mode::calculateForces" );
    Kokkos::fence();
